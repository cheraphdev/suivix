...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        async function addpubchannel(serverID, channelID) {
            ...
            if (!server[serverID]) {
                server[serverID] = {}
                fs.writeFile("./database/channelpub.json", JSON.stringify(server), (err) => {
                    if (err) console.error(err)
                })
            }

            server = require(path.resolve(path.join("./database/channelpub.json")))
            ...
            fs.writeFile(
                path.resolve(path.join("./database/channelpub.json")),
